...
px.plot(x, norm.pdf(x, mean, std), color='red', linestyle='dashed', label="Normal Dist.")
px.title("Distribution of Vehicle Year and Its Effect on Sales", fontsize=14)
px.xlabel("Model Year")
px.ylabel("Total Vehicle Sales")
px.legend()
st.pyplot(fig3)
...
px.xlabel('Odometer Reading (miles)')
px.ylabel('Purchase Price ($)')
px.title('Vehicle Purchase Price vs. Odometer Reading')
max_miles = df['odometer'].max()
max_price = df['price'].max()
px.xlim(0, 500000)
px.ylim(0, 150000)
px.xticks(np.arange(0, 500001, 50000), rotation=45)
px.gca().xaxis.set_major_formatter(px.FuncFormatter(lambda x, _: f'{int(x):,}'))
px.yticks(np.arange(0, 100001, 5000))
px.gca().yaxis.set_major_formatter(px.FuncFormatter(lambda x, _: f'${int(x):,}')) 
st.pyplot(fig4)